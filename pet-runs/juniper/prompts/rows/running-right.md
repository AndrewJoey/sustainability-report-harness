Create one horizontal animation strip for Codex pet `juniper`, state `running-right`.
...
Identity: same pet in every frame: Compact teal-and-cream red panda analyst; leaf-shaped ears, warm amber eyes, orderly dark tail bands, calm curious expression, precise and trustworthy personality, no clothing and no props.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `sticker`: Polished sticker mascot with bold clean shapes, crisp outline, flat colors, and minimal highlight detail. User style notes: Polished soft-edged sticker illustration with restrained dimensional shading, clean dark outline, large readable features, consistent compact whole-body silhouette..
