Create Codex pet row `running-right` for `juniper`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Compact teal-and-cream red panda analyst; leaf-shaped ears, warm amber eyes, orderly dark tail bands, calm curious expression, precise and trustworthy personality, no clothing and no props.. Preserve silhouette, face, palette, material, proportions, markings, and props.
